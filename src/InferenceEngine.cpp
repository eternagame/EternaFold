...
inline RealT InferenceEngine::ScorePairedUnpositionEvidenceRaw(int which_data,int i) const
{
...
#if defined(PARAMS_EVIDENCE)
inline RealT InferenceEngine::ScoreUnpairedPositionEvidenceRaw(int which_data,int i) const
{
...
#if defined(PARAMS_EVIDENCE)
RealT InferenceEngine::ScoreUnpairedPositionEvidence(int i) const
{
   RealT sum = 0;
...
#if defined(PARAMS_EVIDENCE)
RealT InferenceEngine::ScorePairedPositionEvidence(int i) const
{
   RealT sum = 0;
   for (int n = 0; n < num_data_sources; n++)
...
#if defined(PARAMS_EVIDENCE)
RealT InferenceEngine::ScoreUnpairedEvidence(int i, int j) const
{
    RealT sum = 0; 
...
void InferenceEngine::FillScores(typename std::vector<std::pair<RealT, RealT> >::iterator begin, typename std::vector<std::pair<RealT, RealT> >::iterator end, RealT value)
{
    while (begin != end)
    {
        begin->first = value;
...
void InferenceEngine::FillCounts(typename std::vector<std::pair<RealT,RealT> >::iterator begin, typename std::vector<std::pair<RealT,RealT> >::iterator end, RealT value)
{
    while (begin != end)
    {
        begin->second = value;
...
int InferenceEngine::ComputeRowOffset(int i, int N) const
{
...
bool InferenceEngine::IsComplementary(int i, int j) const
{
...
void InferenceEngine::RegisterParameters(ParameterManager &parameter_manager)
{
    char buffer[1000];
    char buffer2[1000];
...
double InferenceEngine::LogGammaProb(RealT data, int which_data, int isUnpaired, int seq)
{
...
void InferenceEngine::LoadSequence(const SStruct &sstruct)
{
    cache_initialized = false;
...
void InferenceEngine::UpdateEvidenceStructures(int which_data)
{
...
void InferenceEngine::InitializeCache()
{
...
void InferenceEngine::InitializeCacheESS()
{
    if (cache_initialized) return;
    cache_initialized = true;
...
void InferenceEngine::ComputeProfileScore(RealT &profile_score, const int *pos, int dimensions, std::pair<RealT,RealT> *table)
{
    profile_score = 0;
...
void InferenceEngine::LoadValues(const std::vector<RealT> &values)
{
...
std::vector<RealT> InferenceEngine::GetCounts()
{
...
void InferenceEngine::ClearCounts()
{
    // clear counts for physical parameters
    for (size_t i = 0; i < parameter_manager->GetNumLogicalParameters(); i++)
    {
...
void InferenceEngine::FinalizeCounts()
{
#if FAST_HELIX_LENGTHS
...
void InferenceEngine::FinalizeCountsESS()
{
#if FAST_HELIX_LENGTHS
...
void InferenceEngine::ConvertProfileCount(const RealT &profile_score, const int *pos, int dimensions, std::pair<RealT,RealT> *table)
{
    // consider all sequences
    for (int k = 0; k < N; k++)
    {
...
void InferenceEngine::UseLoss(const std::vector<int> &true_mapping, RealT example_loss)
{
...
void InferenceEngine::UseConstraints(const std::vector<int> &true_mapping)
{
...
inline RealT InferenceEngine::ScoreJunctionA(int i, int j) const
{
...
inline void InferenceEngine::CountJunctionA(int i, int j, RealT value)
{
    Assert(0 < i && i <= L && 0 <= j && j < L, "Invalid indices.");
...
inline RealT InferenceEngine::ScoreJunctionB(int i, int j) const
{
...
inline void InferenceEngine::CountJunctionB(int i, int j, RealT value)
{
    Assert(0 < i && i < L && 0 < j && j < L, "Invalid indices.");
...
inline RealT InferenceEngine::ScoreBasePair(int i, int j) const
{
...
inline void InferenceEngine::CountBasePair(int i, int j, RealT value)
{
    Assert(0 < i && i <= L && 0 < j && j <= L && i != j, "Invalid base-pair");
...
inline RealT InferenceEngine::ScoreBasePairEvidence(int i, int j) const
{
    return ScoreBasePair(i,j) + ScorePairedPositionEvidence(i) + ScorePairedPositionEvidence(j);  
}


inline void InferenceEngine::CountBasePairEvidence(int i, int j, RealT value)
{
    Assert(0 < i && i <= L && 0 < j && j <= L && i != j, "Invalid base-pair");
...
inline RealT InferenceEngine::ScoreHairpinEvidence(int i, int j) const
{
	return ScoreHairpin(i,j) + ScoreUnpairedEvidence(i,j);
}

inline void InferenceEngine::CountHairpinEvidence(int i, int j, RealT value)
{  
...
inline RealT InferenceEngine::ScoreHairpin(int i, int j) const
{
...
inline void InferenceEngine::CountHairpin(int i, int j, RealT value)
{
    Assert(0 < i && i + C_MIN_HAIRPIN_LENGTH <= j && j < L, "Hairpin boundaries invalid.");
...
inline RealT InferenceEngine::ScoreHelix(int i, int j, int m) const
{
...
inline void InferenceEngine::CountHelix(int i, int j, int m, RealT value)
{
    Assert(0 <= i && i + 2 * m <= j && j <= L, "Helix boundaries invalid.");
    Assert(2 <= m && m <= D_MAX_HELIX_LENGTH, "Helix length invalid.");
...
inline RealT InferenceEngine::ScoreHelixEvidence(int i, int j, int m) const
{
    // First, i >= 0 && j <= L are obvious sanity-checks to make sure that
    // things are within range.  The check that i+2*m <= j ensures that there
    // are enough nucleotides to allow a helix of length m.
...
inline void InferenceEngine::CountHelixEvidence(int i, int j, int m, RealT value)
{
    Assert(0 <= i && i + 2 * m <= j && j <= L, "Helix boundaries invalid.");
    Assert(2 <= m && m <= D_MAX_HELIX_LENGTH, "Helix length invalid.");
...
inline RealT InferenceEngine::ScoreSingleNucleotidesEvidence(int i, int j, int p, int q) const
{
	return ScoreSingleNucleotides(i,j,p,q) + ScoreUnpairedEvidence(i,p) + ScoreUnpairedEvidence(q,j);
}

inline void InferenceEngine::CountSingleNucleotidesEvidence(int i, int j, int p, int q, RealT value)
{
...
inline RealT InferenceEngine::ScoreSingleNucleotides(int i, int j, int p, int q) const
{
...
inline void InferenceEngine::CountSingleNucleotides(int i, int j, int p, int q, RealT value)
{
    Assert(0 < i && i <= p && p + 2 <= q && q <= j && j < L, "Single-branch loop boundaries invalid.");
...
inline RealT InferenceEngine::ScoreSingleEvidence(int i, int j, int p, int q) const
{
    const int l1 = p - i;
    const int l2 = j - q;
...
inline void InferenceEngine::CountSingleEvidence(int i, int j, int p, int q, RealT value)
{
    const int l1 = p - i;
    const int l2 = j - q;
...
inline RealT InferenceEngine::ScoreSingle(int i, int j, int p, int q) const
{
    const int l1 = p - i;
    const int l2 = j - q;
...
inline void InferenceEngine::CountSingle(int i, int j, int p, int q, RealT value)
{
    const int l1 = p - i;
    const int l2 = j - q;
...
inline int InferenceEngine::EncodeTraceback(int i, int j) const
{
...
inline std::pair<int,int> InferenceEngine::DecodeTraceback(int s) const
{
...
void InferenceEngine::ComputeViterbi()
{
    InitializeCache();
...
// inline RealT InferenceEngine::GetViterbiScore() const
// {
//     return F5v[L];
// }
...
std::vector<int> InferenceEngine::PredictPairingsViterbi() const
{
...
std::vector<RealT> InferenceEngine::ComputeViterbiFeatureCounts()
{
...
void InferenceEngine::ComputeInside()
{
    InitializeCache();
...
void InferenceEngine::ComputeOutside()
{
    InitializeCache();
...
RealT InferenceEngine::ComputeLogPartitionCoefficient() const
{
...
std::vector<RealT> InferenceEngine::ComputeFeatureCountExpectations()
{
#if SHOW_TIMINGS
    double starting_time = GetSystemTime();
#endif
...
std::vector<RealT> InferenceEngine::ComputeFeatureCountExpectationsESS()
{
#if SHOW_TIMINGS
    double starting_time = GetSystemTime();
#endif
...
void InferenceEngine::ComputePosterior()
{ 
    posterior.clear();
...
void InferenceEngine::ComputePosteriorESS()
{ 
    posterior.clear();
    posterior.resize(SIZE, RealT(0));
...
std::vector<int> InferenceEngine::PredictPairingsPosterior(const RealT gamma) const
{
...
std::vector<int> InferenceEngine::PredictPairingsPosteriorCentroid(const RealT gamma) const
{
    Assert(gamma > 0, "Non-negative gamma expected.");
...
RealT *InferenceEngine::GetPosterior(const RealT posterior_cutoff) const
{
...
void InferenceEngine::ComputeInsideESS() 
{
    InitializeCacheESS();
...
void InferenceEngine::ComputeOutsideESS() 
{
    InitializeCacheESS();
...
RealT InferenceEngine::ComputeLogPartitionCoefficientESS() const
{
...
std::vector<RealT> InferenceEngine::ComputeESS()
{
#if SHOW_TIMINGS
    double starting_time = GetSystemTime();
#endif
...
RealT InferenceEngine::ComputeGammaMLESum(std::vector<int> ev_cpd_id, bool ignorePairing, bool usePosterior, int which_data) 
{
    RealT sum1 = 0; 
    int i = ev_cpd_id[0];
    int j = ev_cpd_id[1];
...
std::vector<RealT> InferenceEngine::ComputeGammaMLESS(std::vector<int> ev_cpd_id, bool ignoreZeros, bool useMLE, int which_data) 
{
...
std::vector<RealT> InferenceEngine::ComputeGammaMLEESS(std::vector<int> ev_cpd_id, bool ignoreZeros, bool useMLE, int which_data) 
{
    std::vector<RealT> result;
...
RealT InferenceEngine::GetNumExamplesSeqPairing(std::vector<int> ev_cpd_id, bool ignoreZeros, int which_data) 
{
    RealT sum = 0; 
    int i = ev_cpd_id[0];
    int j = ev_cpd_id[1];
...
RealT InferenceEngine::GetNumExamplesSeq(std::vector<int> ev_cpd_id, bool ignoreZeros, int which_data) 
{
    RealT sum = 0; 
    int i = ev_cpd_id[0];
    int j = ev_cpd_id[1];
...
int InferenceEngine::AreZerosInSeqPairing(int id_base, int id_pairing, int which_data) 
{
...
int InferenceEngine::AreZerosInSeq(int id_base, int which_data)  
{
